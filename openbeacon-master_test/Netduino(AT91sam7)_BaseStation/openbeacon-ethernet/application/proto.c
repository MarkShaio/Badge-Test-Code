...
/* Testing localiser packet structure
	  l_Beacon.seq = g_Beacon.log.pkt.p.tracker.seq;
	  l_Beacon.flags = g_Beacon.log.pkt.flags;
	  l_Beacon.strength = g_Beacon.log.pkt.p.tracker.strength;
	  l_Beacon.oid = g_Beacon.log.pkt.oid;
	  l_Beacon.protocol = g_Beacon.log.pkt.proto;
	  l_Beacon.reserved = g_Beacon.log.pkt.p.tracker.reserved;
	  l_Beacon.size = 0x10;//swapshort (sizeof (l_Beacon));
	  l_Beacon.crc = env_icrc16 ( (u_int8_t*)&l_Beacon,0x0E);
*/
	...
	{
	for(i=0; i<16; i++){
	debug_printf("%02x", g_Beacon.log.byte[i]);
        }
	debug_printf("\n");
	...
	/*Debugging - take apart packet*/
	  debug_printf("proto: 0x%02X\n", g_Beacon.log.pkt.proto & 0xFF);
          debug_printf("oid: 0x%04X\n", swapshort(g_Beacon.log.pkt.oid & 0xFFFF));
          debug_printf("flags: 0x%02X\n", g_Beacon.log.pkt.flags & 0xFF);

          debug_printf("strength: 0x%02X\n", g_Beacon.log.pkt.p.tracker.strength & 0xFF);
          debug_printf("oid_last_seen: 0x%04X\n", swapshort(g_Beacon.log.pkt.p.tracker.oid_last_seen & 0xFFFF));
          debug_printf("power_up_count: 0x%04X\n", swapshort(g_Beacon.log.pkt.p.tracker.powerup_count & 0xFFFF));
          debug_printf("reserved: 0x%02X\n", g_Beacon.log.pkt.p.tracker.reserved & 0xFF);
          debug_printf("seqno: 0x%08X\n", swaplong(g_Beacon.log.pkt.p.tracker.seq));
          debug_printf("crc: 0x%04X\n", swapshort(g_Beacon.log.pkt.crc & 0xFFFF));
...
vnRFtaskRxTx (void *parameter)
{
  ...
  vTaskDelay (500 / portTICK_RATE_MS); //Fixes initialisation problem C.R.
...
PtInitProtocol (void)
{
  ...
  rf_lastblink[0] = 0; //rf_lastblink[1] = 0;

  duplicate_backlog_pos = rf_duplicates[0] = 0; //rf_duplicates[1] = 0;
