...
vNetworkResetDefaultSettings (void)
{
  ...
  env.e.reader_id = 3;
  env.e.ip_autoconfig = IP_AUTOCONFIG_STATIC_IP;
  env.e.ip_server_port = 2342;
  env.e.filter_duplicates = pdTRUE;
  IP4_ADDR (&env.e.ip_host, 130, 102, 70, 46);
  IP4_ADDR (&env.e.ip_netmask, 255, 255, 255, 128);
  IP4_ADDR (&env.e.ip_gateway, 130, 102, 70, 1);
  IP4_ADDR (&env.e.ip_server, 130, 102, 128, 123);
}
