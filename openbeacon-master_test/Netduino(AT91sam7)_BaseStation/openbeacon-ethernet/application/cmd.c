...
vCmdHelp (void)
{
  debug_printf ("\n\nOpenBeacon.org PoE Ethernet II v" PROGRAM_VERSION "\n"
		...
		"\t'j' - green LED ('l[enable=0, disable=1]')\n"
...
vCmdProcess (const char *cmdline)
{
  ...
  switch (cmd)
	...
	{
	  t = atoiEx (cmdline) > 0;
	  debug_printf ("green_led=%i\n", t);
	  vLedSetGreen (t);
	}
      else
	debug_printf ("usage: 'l' - green LED ('l[enable=0, disable=1]')\n");
      break;
...
vCmdTask (void *pvParameters)
{
  ...
  for (;;)
      ...
      if (vUSBRecvByte (&cByte, 1, 100))
	{
      ...
      else if (AT91F_PIO_GetInput (EXT_BUTTON_PIO) & !EXT_BUTTON_PIN) //netduino button seems to be tripped
	{
	  if (t < 30)
	    t++;
	  else
